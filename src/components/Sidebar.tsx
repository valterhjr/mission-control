...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        await (api as Record<string, (...args: unknown[]) => Promise<unknown>>).getSessions?.(1);
    ...
    <aside className="mc-sidebar">
      ...
      <div className="mc-sidebar-brand">
        ...
        <div className="mc-sidebar-brandtext">
          <span className="mc-sidebar-title">Mission <span style={{ color: 'var(--mc-accent)' }}>Control</span></span>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') window.location.href = item.href; }}
            >
      ...
      <div className="mc-sidebar-footer">
        <div className="mc-sidebar-status">
          <span className={`mc-dot ${online ? "mc-dot-online" : "mc-dot-offline"}`} />
          <span className="mono" style={{ fontSize: 11 }}>
            {online ? t("Gateway Online") : t("Gateway Offline")}
          </span>
        </div>
      </div>
